...
void pid_init(cpid_t *pid, float p, float i, float d, uint8_t f, int32_t dt_ms)
{
    ...
    pid->d_max = INT32_MAX;
    pid->d_min = INT32_MIN;
    pid->e_last = 0; //wartość uchyb regulacji w poprzedniej iteracji sterownika
    pid->sum = 0;
    pid->total_max = 4095;
    pid->total_min = 0;
    pid->dt_ms = dt_ms;
...
int32_t pid_calc(cpid_t *pid, int32_t mv, int32_t dv)
{
    ...
    i += pid->i *( ( ( pid->e_last + e ) /2 ) *pid->dt_ms );    //Trapezowe
    if (i > pid->i_max)
        i = pid->i_max;
    else if (i < pid->i_min)
        i = pid->i_min;
    ...
    //PAMIETAJ O SKALOWANIU CZASU WYKONYWANIA PETLI DO SEKUND
    d = pid->d * ( (mv - pid->e_last) * pid->dt_ms);
    if (d > pid->d_max)
        d = pid->d_max;
    else if (d < pid->d_min)
        d = pid->d_min;
